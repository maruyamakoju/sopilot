...
def test_api_key():
    """Test API key"""
    return api_key_store.generate_key(name="test-key", permissions={"read": True, "write": True, "admin": True})
...
def test_upload_claim_invalid_key(client):
    ...
    response = client.post("/claims/upload", headers={"X-API-Key": "invalid_key_123"})
    assert response.status_code == 401


def test_upload_claim_no_file(client, test_api_key):
    """Test upload without file"""
    response = client.post("/claims/upload", headers={"X-API-Key": test_api_key})
...
def test_get_claim_status_not_found(client, test_api_key):
    ...
    response = client.get("/claims/nonexistent/status", headers={"X-API-Key": test_api_key})
...
def test_get_claim_status_success(client, test_api_key):
    ...
    upload_response = client.post(
        "/claims/upload", headers={"X-API-Key": test_api_key}, files={"video": ("test2.mp4", video_data, "video/mp4")}
    )
    claim_id = upload_response.json()["claim_id"]

    # Get status
    response = client.get(f"/claims/{claim_id}/status", headers={"X-API-Key": test_api_key})
...
def test_get_assessment_not_ready(client, test_api_key):
    ...
    video_data = b"MOCK_VIDEO" * 100
    upload_response = client.post(
        "/claims/upload", headers={"X-API-Key": test_api_key}, files={"video": ("test3.mp4", video_data, "video/mp4")}
    )
    claim_id = upload_response.json()["claim_id"]

    # Try to get assessment immediately
    response = client.get(f"/claims/{claim_id}/assessment", headers={"X-API-Key": test_api_key})
...
def test_get_queue_success(client, test_api_key):
    ...
    response = client.get("/reviews/queue", headers={"X-API-Key": test_api_key}, params={"limit": 10})
...
def test_get_queue_filtered(client, test_api_key):
    ...
    response = client.get(
        "/reviews/queue", headers={"X-API-Key": test_api_key}, params={"priority": "URGENT", "limit": 5}
    )
...
def test_submit_review_invalid_decision(client, test_api_key):
    ...
    response = client.post(
        ...
        json={"decision": "INVALID", "reasoning": "This is a test reasoning with sufficient length"},
    )
    assert response.status_code == 422  # Validation error
...
def test_submit_review_short_reasoning(client, test_api_key):
    ...
    response = client.post(
        ...
        headers={"X-API-Key": test_api_key},
        params={"reviewer_id": "test_reviewer"},
        json={"decision": "APPROVE", "reasoning": "Short"},
    )
    assert response.status_code == 422  # Validation error
...
def test_submit_review_claim_not_found(client, test_api_key):
    ...
    response = client.post(
        ...
        headers={"X-API-Key": test_api_key},
        params={"reviewer_id": "test_reviewer"},
        json={"decision": "APPROVE", "reasoning": "This is a test reasoning with sufficient length for validation"},
    )
    assert response.status_code == 404
...
def test_get_history_not_found(client, test_api_key):
    ...
    response = client.get("/reviews/nonexistent/history", headers={"X-API-Key": test_api_key})
    assert response.status_code == 404
...
def test_get_history_success(client, test_api_key):
    ...
    video_data = b"MOCK_VIDEO" * 100
    upload_response = client.post(
        "/claims/upload", headers={"X-API-Key": test_api_key}, files={"video": ("test4.mp4", video_data, "video/mp4")}
    )
    claim_id = upload_response.json()["claim_id"]

    # Get history
    response = client.get(f"/reviews/{claim_id}/history", headers={"X-API-Key": test_api_key})
...
def test_bearer_token_auth(client, test_api_key):
    ...
    response = client.get("/metrics", headers={"Authorization": f"Bearer {test_api_key}"})
    assert response.status_code == 200


def test_invalid_bearer_token(client):
    """Test invalid Bearer token on endpoint requiring auth"""
    response = client.get("/claims/nonexistent/status", headers={"Authorization": "Bearer invalid_token"})
    assert response.status_code == 401
...
def test_read_only_permission(client):
    ...
    readonly_key = api_key_store.generate_key(name="readonly-test", permissions={"read": True, "write": False})
    ...
    response = client.post(
        "/claims/upload", headers={"X-API-Key": readonly_key}, files={"video": ("test.mp4", b"data", "video/mp4")}
    )
...
def test_full_workflow(client, test_api_key):
    ...
    # 2. Check status
    status_response = client.get(f"/claims/{claim_id}/status", headers={"X-API-Key": test_api_key})
    ...
    assessment_response = client.get(f"/claims/{claim_id}/assessment", headers={"X-API-Key": test_api_key})
    # Either ready or not ready
    assert assessment_response.status_code in [200, 404]

    # 5. Get history
    history_response = client.get(f"/reviews/{claim_id}/history", headers={"X-API-Key": test_api_key})
