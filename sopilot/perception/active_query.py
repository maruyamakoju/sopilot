...
class ReviewQueue:
    ...
    def __init__(
        ...
        self._frames_root = Path(frames_root) if frames_root else None
    ...
    def _save(self) -> None:
        """pending + reviewed をJSON ファイルに保存する (best-effort)。"""
        if self._persist_path is None:
            return
        try:
            self._persist_path.parent.mkdir(parents=True, exist_ok=True)
            data = {
                "pending": [dataclasses.asdict(i) for i in self._pending],
                "reviewed": [dataclasses.asdict(i) for i in self._reviewed],
            }
            self._persist_path.write_text(json.dumps(data, indent=2), encoding="utf-8")
        except Exception:
            logger.exception("ReviewQueue: failed to save to %s", self._persist_path)
    ...
    def _load(self) -> None:
        ...
        try:
            ...
            def _item(d: dict) -> ReviewItem:
                d.setdefault("frame_path", "")
                return ReviewItem(**d)

            self._pending = [_item(d) for d in data.get("pending", [])]
            self._reviewed = [_item(d) for d in data.get("reviewed", [])]
        except Exception:
            logger.exception("ReviewQueue: failed to load from %s", self._persist_path)
    ...
    def maybe_add(
        ...
        with self._lock:
            ...
            # フレーム JPEG を保存
            if frame_jpeg and self._frames_root is not None:
                try:
                    self._frames_root.mkdir(parents=True, exist_ok=True)
                    frame_file = self._frames_root / f"{new_item.review_id}.jpg"
                    frame_file.write_bytes(frame_jpeg)
                    new_item.frame_path = str(frame_file)
                except Exception:
                    pass
    ...
    def get_review_history(self, limit: int = 50) -> list[dict[str, Any]]:
        """レビュー済みアイテムの時系列リストを返す (最新 limit 件)。"""
        with self._lock:
            items = sorted(self._reviewed, key=lambda x: x.created_at)
